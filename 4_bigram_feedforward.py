...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_heads, head_size):
        super().__init__()
        self.heads = nn.ModuleList([Head(head_size) for _ in range(n_heads)])  # create n_heads heads

    def forward(self, x):
        return torch.cat([h(x) for h in self.heads], dim=-1)  # concatenate the outputs of each head
...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.sa_heads = MultiHeadAttention(n_heads=4, head_size=n_embd // 4)  # multihead self-attention
    ...
    def forward(self, idx, targets=None):
        ...
        x = self.ffwd(x)  # apply the feedforward block (B, T, C)
